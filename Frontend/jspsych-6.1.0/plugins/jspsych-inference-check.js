jsPsych.plugins['inference-check'] = (function() {
    ...
    jsPsych.pluginAPI.registerPreload('inference-check', 'main_stimulus', 'image');
    ...
    plugin.trial = function(display_element, trial) {
      ...
      var html = '<div id="jspsych-inference-check-wrapper" style="margin: 100px 0px;">';
      ...
      for (let pair of trial.stimuli_and_text) {
        ...
        html += '<div class="jspsych-inference-check-container" style="position:relative; margin: 0 auto 3em auto; ';
        if(trial.slider_width !== null){
          html += 'width:'+trial.slider_width+'px;';
        }
        html += '">';
        html += '<input type="range" value="'+trial.start+'" min="'+trial.min+'" max="'+trial.max+'" step="'+trial.step+'" style="width: 100%;" id="jspsych-inference-check-response-' + i + '"></input>';
        html += '<div>'
      ...
      html += '<button id="jspsych-inference-check-next" class="jspsych-btn" '+ (trial.require_movement ? "disabled" : "") + '>'+trial.button_label+'</button>';
  ...
  if(trial.require_movement){
    display_element.querySelector('#jspsych-inference-check-response-' + trial.stimuli_and_text.length).addEventListener('change', function(){
      display_element.querySelector('#jspsych-inference-check-next').disabled = false;
    })
  }
  
  // Add event listener to the submit button
  display_element.querySelector('#jspsych-inference-check-next').addEventListener('click', function() {
    ...
    for (var i = 1; i <= trial.stimuli_and_text.length; i++) {
      response.responses.push(display_element.querySelector('#jspsych-inference-check-response-' + i).value);
    }
    ...
    if(trial.response_ends_trial){
      ...
      display_element.querySelector('#jspsych-inference-check-next').disabled = true;
    }
  });
  ...
  if (trial.stimulus_duration !== null) {
    jsPsych.pluginAPI.setTimeout(function() {
      display_element.querySelector('#jspsych-inference-check-stimulus').style.visibility = 'hidden';
