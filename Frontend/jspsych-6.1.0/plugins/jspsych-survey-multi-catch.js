jsPsych.plugins["survey-multi-catch"] = (function () {
  ...
  plugin.info = {
    ...
    description:
      "A plugin for multiple-choice survey questions with instruction looping and error catching",
    parameters: {
      questions: {
        ...
        nested: {
          prompt: {
            ...
            default: undefined,
            description:
              "The strings that will be associated with a group of options.",
          },
          options: {
            type: jsPsych.plugins.parameterType.STRING,
            pretty_name: "Options",
          ...
          required: {
            ...
            default: false,
            description:
              "Subject will be required to pick an option for each question.",
          },
          horizontal: {
            type: jsPsych.plugins.parameterType.BOOL,
      ...
      correct_answers: {
        ...
        default: {},
        description:
          "An object containing the correct answers for each question",
      },
      instructions: {
        ...
        default: null,
        description:
          "HTML-formatted string containing the instructions to display",
      },
    },
  };
  ...
  plugin.trial = function (display_element, trial) {
    ...
    html +=
      ".jspsych-survey-multi-catch-question { margin-top: 2em; margin-bottom: 2em; text-align: left; }" +
    ...
    if (trial.preamble !== null) {
      html +=
        '<div id="jspsych-survey-multi-catch-preamble" class="jspsych-survey-multi-catch-preamble">' +
        trial.preamble +
        "</div>";
    }
    ...
    for (var i = 0; i < trial.questions.length; i++) {
      ...
      html +=
        '<div id="jspsych-survey-multi-catch-' +
        question_id +
        '" class="' +
      ...
      // add question text
      html +=
        '<p class="jspsych-survey-multi-catch-text survey-multi-catch">' +
        question.prompt;
      ...
      for (var j = 0; j < question.options.length; j++) {
        var option_id_name =
          "jspsych-survey-multi-catch-option-" + question_id + "-" + j;
        var input_name = "jspsych-survey-multi-catch-response-" + question_id;
        var input_id =
          "jspsych-survey-multi-catch-response-" + question_id + "-" + j;
        ...
        html +=
          '<div id="' +
          option_id_name +
          '" class="jspsych-survey-multi-catch-option">';
        html +=
          '<label class="jspsych-survey-multi-catch-text" for="' +
          input_id +
          '">' +
          question.options[j] +
          "</label>";
      ...
      if (formElement) {
        formElement.addEventListener("submit", function (event) {
          ...
          if (all_correct) {
            ...
            display_element
              .querySelector("#startPhase1Button")
              .addEventListener("click", function () {
                var trial_data = {
                  rt: response_time,
                  ...
                  failed_submission_timestamps: JSON.stringify(
                    failedSubmissionData.timestamps,
                  ),
                };
