...
jsPsych.plugins["multi-image-slider-response"] = (function () {
  ...
  plugin.trial = function (display_element, trial) {
    var html =
      '<div id="jspsych-multi-image-slider-response-wrapper" style="margin: 100px 0px;">';
    ...
    // slider 1 ------------
    html +=
      '<div class="jspsych-multi-image-slider-response-container" style="position:relative; margin: 0 auto 3em auto; ';
    ...
    html +=
      ...
      '" step="' +
      trial.step +
      '" style="width: 100%;" id="jspsych-multi-image-slider-response-response"></input>';
    html += "<div>";
    ...
    html +=
      ...
      '" step="' +
      trial.step +
      '" style="width: 100%;" id="jspsych-multi-image-slider-response-response2"></input>';
    html += "<div>";
    for (var j = 0; j < trial.labels.length; j++) {
      var width = 100 / (trial.labels.length - 1);
      var left_offset = j * (100 / (trial.labels.length - 1)) - width / 2;
    ...
    html +=
      ...
      '" step="' +
      trial.step +
      '" style="width: 100%;" id="jspsych-multi-image-slider-response-response3"></input>';
    html += "<div>";
    for (var j = 0; j < trial.labels.length; j++) {
      var width = 100 / (trial.labels.length - 1);
      var left_offset = j * (100 / (trial.labels.length - 1)) - width / 2;
    ...
    // add submit button
    html +=
      '<button id="jspsych-multi-image-slider-response-next" class="jspsych-btn" ' +
    ...
    if (trial.require_movement) {
      display_element
        .querySelector("#jspsych-multi-image-slider-response-response3")
        .addEventListener("change", function () {
          display_element.querySelector(
            "#jspsych-multi-image-slider-response-next",
          ).disabled = false;
        });
    }

    display_element
      .querySelector("#jspsych-multi-image-slider-response-next")
      .addEventListener("click", function () {
        // measure response time
        var endTime = performance.now();
        response.rt = endTime - startTime;
        response.response1 = display_element.querySelector(
          "#jspsych-multi-image-slider-response-response",
        ).value;
        response.response2 = display_element.querySelector(
          "#jspsych-multi-image-slider-response-response2",
        ).value;
        response.response3 = display_element.querySelector(
          "#jspsych-multi-image-slider-response-response3",
        ).value;
        console.log(
          "response1: " +
            response.response1 +
    ...
    if (trial.stimulus_duration !== null) {
      jsPsych.pluginAPI.setTimeout(function () {
        display_element.querySelector(
          "#jspsych-multi-image-slider-response-stimulus",
