jsPsych.plugins["valence-check-all"] = (function () {
  ...
  plugin.trial = function (display_element, trial) {
    ...
    // Add submit button
    html +=
      '<button id="jspsych-valence-check-all-next" class="jspsych-btn">Next</button>';
    ...
    display_element
      .querySelector("#jspsych-valence-check-all-next")
      .addEventListener("click", function () {
        // Retrieve response values
        for (var i = 0; i < trial.stimuli.length; i++) {
          var slider_id = "jspsych-valence-check-all-slider-" + i;
          response[trial.stimuli[i].id] = display_element.querySelector(
            "#" + slider_id,
          ).value;
        }
